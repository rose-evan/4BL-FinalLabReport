...
\begin{table}[H]
\centering
\caption{Clear pipe coil positions, trigger times, and segment velocities for Trial 1.
Heights are measured from the drop point; $v_\text{theory} = \sqrt{2gh}$ evaluated at the
midpoint height of each segment.}
\label{tab:clear_trial1}
\begin{tabular}{cccccc}
\toprule
Coil & Position from top (m) & Height fallen $h$ (m) & Trigger time (s) &
$v_\text{exp}$ (m/s) & $v_\text{theory}$ (m/s) \\
\midrule
1 & 0.310 & 0.100 & 1.630 & ---      & 1.401 \\
2 & 0.225 & 0.185 & 1.684 & 1.574    & 1.904 \\
3 & 0.150 & 0.260 & 1.710 & 2.885    & 2.257 \\
4 & 0.065 & 0.345 & 1.740 & 2.833    & 2.600 \\
\bottomrule
\end{tabular}
\end{table}

The measured average velocity across the full span (coils 1--4) was
$\bar{v}_\text{exp} = 2.227$~m/s, compared to a theoretical value of
$v_\text{theory} = \sqrt{2g \cdot h_\text{mid}} = 2.192$~m/s, yielding a percent error of
$1.59\%$. This low error validates that the coil timing system accurately tracks the magnet's
position.
...
% ------------------------------------------------------------------
% TABLE 3 — Multi-trial summary (TODO: fill with trials 2 & 3)
% ------------------------------------------------------------------
\begin{table}[H]
\centering
\caption{Terminal velocity and damping coefficient across all three copper pipe trials.
$v_t$ is taken as the mean of the last two segment velocities in each trial.
$b = mg/v_t$ with $m = 0.023$~kg and $g = 9.81$~m/s$^2$.}
\label{tab:multi_trial}
\begin{tabular}{cccc}
\toprule
Trial & $v_t$ (m/s) & $b$ (kg/s) & Notes \\
\midrule
1     & 0.1092      & 2.066      &       \\
2     & % TODO      & % TODO     &       \\
3     & % TODO      & % TODO     &       \\
...
\noindent Applied to the terminal velocity segment ($\Delta x = 0.046$~m,
$\Delta t = 0.42$~s), this gives $\delta v_t = \pm 0.0097$~m/s. The uncertainty in the
...
\noindent yielding $\delta b = \pm 0.20$~kg/s. The final reported values for Trial 1 are:

\begin{equation*}
    v_t = 0.1092 \pm 0.0097~\text{m/s}, \qquad b = 2.07 \pm 0.20~\text{kg/s}
\end{equation*}
